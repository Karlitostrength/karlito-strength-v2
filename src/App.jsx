...
function LibraryScreen({ isCoach }) {
  const [exercises, setExercises] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  return (
    <div style={s.screen}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto", paddingBottom: 5 }}>
        {categories.map(cat => (
          <div key={cat} onClick={() => setActiveCategory(cat)}
            style={{ ...s.pill(activeCategory === cat), padding: "6px 12px", whiteSpace: "nowrap" }}>
            {cat}
          </div>
        ))}
      </div>
      ...
      {filteredEx.length === 0 ? (
        <div style={{ ...s.card, textAlign: "center", padding: 32 }}>
          <div style={{ fontSize: 13, color: "var(--gray)" }}>No exercises found.</div>
        </div>
      ) : (
        filteredEx.map(ex => {
          ...
          return (
            <div key={ex.id} style={{ ...s.card, marginBottom: 16, padding: "16px 12px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                ...
                {isCoach && (
                  <button onClick={() => deleteEx(ex.id)} style={{background: 'transparent', border: 'none', color: 'var(--red)', fontSize: 16}}>✕</button>
                )}
              </div>
