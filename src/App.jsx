...
function SetRow({ setIdx, log, plannedWeight, plannedReps, onUpdate, onToggleDone, suggestedWeight }) {
  const inputStyle = {
...
const LIBRARY_CATEGORIES = ["All", "Swing", "Snatch", "Clean", "Jerk", "Squat", "Deadlift", "Bench", "Accessory"];
...
function LibraryScreen({ authUser, isCoach }) {
  const [exercises, setExercises] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ name: "", category: "Swing", youtube_url: "", description: "" });
  ...
  const saveExercise = async () => {
    ...
    try {
      if (editingId) {
        ...
        await supabase.from("exercise_library").insert({ ...form, created_by: authUser.id });
      }
      setForm({ name: "", category: "Swing", youtube_url: "", description: "" });
      setEditMode(false);
      setEditingId(null);
      await loadExercises();
  ...
  LIBRARY_CATEGORIES.forEach(c => {
    catCounts[c] = c === "All" ? exercises.length : exercises.filter(e => e.category === c).length;
  });

  const catColor = { Swing: "#4a9eff", Snatch: "#f0a020", Clean: "#c41e1e", Jerk: "#e8d5a0", Squat: "#a0e8d5", Deadlift: "#e8a0d5", Bench: "#d5e8a0", Accessory: "#888880" };
  ...
  return (
    <div style={s.screen}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        {isCoach && !editMode && (
          <button onClick={() => { setEditMode(true); setEditingId(null); setForm({ name: "", category: "Swing", youtube_url: "", description: "" }); }}
      ...
      {isCoach && editMode && (
        <div style={{ ...s.card, marginBottom: 16, borderColor: "var(--red-dim)" }}>
          ...
          <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            placeholder="e.g. Double Kettlebell Swing" style={{ ...s.input, marginBottom: 12 }} />
          ...
          <label style={s.label}>YOUTUBE LINK (optional)</label>
          ...
          <label style={s.label}>DESCRIPTION / TECHNIQUE NOTES</label>
          <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            placeholder="Key coaching points, common mistakes, progressions..."
            rows={4}
            style={{ ...s.input, resize: "vertical", lineHeight: 1.6, minHeight: 90 }} />

          <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
            <button onClick={saveExercise} disabled={saving || !form.name.trim()}
              style={{ ...s.btn, flex: 1, opacity: !form.name.trim() ? 0.5 : 1 }}>
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 16, paddingBottom: 4 }}>
        {LIBRARY_CATEGORIES.filter(c => catCounts[c] > 0 || c === "All").map(cat => (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    {ytId && <span style={{ fontSize: 12 }}>▶️</span>}
                  </div>
