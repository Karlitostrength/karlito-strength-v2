...
import { sendPushToUser, sendPushToAllUsers } from "../lib/push";
    ...
    <div style={{
      ...
      {showStandards && standards && (
        <div style={{ marginTop: 14, borderTop: `1px solid ${level.color}55`, paddingTop: 12 }}>
          ...
          {Object.entries(standards).map(([pillar, std]) => (
            <div key={pillar} style={{ display: "flex", gap: 10, marginBottom: 8,
              ...
              <div style={{ fontSize: 13, color: "var(--white)", lineHeight: 1.5, fontWeight: 500 }}>{std.label}</div>
            </div>
          ))}
          {level.engineOptions && (
...
function DomSilyPathScreen({ user, authUser }) {
  ...
  const saveTests = async () => {
    ...
    try {
      const updates = {
        pushups: testVals.pushups, inverted_rows: testVals.inverted_rows,
        kb_swing_test: testVals.kb_swing_test, goblet_test: testVals.goblet_test,
        farmers_carry: testVals.farmers_carry, tgu_test: testVals.tgu_test,
        kb_press_test: testVals.kb_press_test, swing_100_test: testVals.swing_100_test,
        snatch_test_kg: testVals.snatch_test_kg, front_squat_test: testVals.front_squat,
        ohs_test: testVals.ohs, weighted_pullup_kg: testVals.weighted_pullup_kg,
        dbl_kb_press_test: testVals.dbl_kb_press_test, gladiator_engine: testVals.gladiator_engine,
        bodyweight, gender,
      };
      const { error } = await supabase.from("profiles").update(updates).eq("id", authUser.id);
      if (error) {
        if (error.message?.includes("column") || error.code === "42703") {
          await supabase.from("profiles").update({ bodyweight, gender }).eq("id", authUser.id);
          setTestError("⚠ Niektóre kolumny testów nie istnieją w bazie.");
          setTestsSaved(true);
  ...
  const progressPct = currentRank > 0
    ...
    : nextGaps ? Math.round((nextGaps.gaps.filter(g => g.passed).length / nextGaps.gaps.length) * 50) : 0;
  ...
  return (
    <div style={s.screen}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: 11, letterSpacing: "0.3em", color: "var(--accent)", marginBottom: 8 }}>DOM SIŁY</div>
        <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 28, fontWeight: 900, letterSpacing: "0.05em" }}>YOUR PATH</div>
        <div style={{ fontSize: 12, color: "var(--gray)", marginTop: 4 }}>FERRUM · SANGUIS · GLORIA</div>
      </div>
      ...
      {currentLevel ? (
        ...
        <div style={{ ...s.card, textAlign: "center", padding: "24px 16px", borderColor: DOM_SILY_LEVELS[0].color + "44" }}>
          <div style={{ fontSize: 36, marginBottom: 8 }}>🔰</div>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: 18, fontWeight: 900, color: "var(--gray)", letterSpacing: "0.1em" }}>NOT YET RANKED</div>
          <div style={{ fontSize: 12, color: "var(--gray2)", marginTop: 8 }}>Complete the tests below to earn your first rank</div>
        </div>
      )}
      ...
      {nextGaps && (
        <div style={{ marginBottom: 16 }}>
          <div style={s.sectionLabel}>NEXT LEVEL — {nextGaps.level.name}</div>
          <div style={{ ...s.card, borderColor: nextGaps.level.color + "44", background: nextGaps.level.color + "08" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <span style={{ fontSize: 28 }}>{nextGaps.level.icon}</span>
              <div>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 16, fontWeight: 900, color: nextGaps.level.color }}>{nextGaps.level.name}</div>
                <div style={{ fontSize: 11, color: "var(--gray)", marginTop: 2 }}>{nextGaps.level.subtitle}</div>
              </div>
            </div>
            {nextGaps.gaps.map(({ pillar, label, passed }) => (
              <div key={pillar} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                ...
                <div>
                  <div style={{ fontSize: 10, color: "var(--gray2)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>{pillar}</div>
                  <div style={{ fontSize: 13, color: passed ? "var(--gray2)" : "var(--text)", textDecoration: passed ? "line-through" : "none", lineHeight: 1.3 }}>{label}</div>
                </div>
              </div>
            ))}
            {nextGaps.level.engineOptions && (
              <div style={{ marginTop: 12, padding: "10px 12px", background: "var(--bg3)", borderRadius: 4, borderLeft: "2px solid var(--accent)" }}>
                <div style={{ fontSize: 10, color: "var(--accent)", letterSpacing: "0.12em", marginBottom: 8, fontFamily: "'Cinzel', serif" }}>ENGINE — CHOOSE ONE</div>
                {nextGaps.level.engineOptions.map((opt, i) => (
                  <div key={i} style={{ fontSize: 12, color: "var(--gray)", marginBottom: 4 }}>{i + 1}. {opt}</div>
                ))}
              </div>
            )}
          </div>
      ...
      <button onClick={() => setShowAllLevels(v => !v)} style={{ ...s.btnGhost, fontSize: 12, marginBottom: 12 }}>
      ...
      {showAllLevels && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
          {[...DOM_SILY_LEVELS].reverse().map(level => (
            <div key={level.id}>
              <LevelCard level={level} isCurrentLevel={level.rank === currentRank} compact={false} showStandards={true} gender={gender} />
              {level.rank > 1 && <div style={{ textAlign: "center", color: "var(--gray2)", fontSize: 16, margin: "2px 0" }}>↑</div>}
            </div>
          ))}
        </div>
      )}
      ...
      {showTests && (
        <div style={{ ...s.card, borderColor: "rgba(232,213,160,0.3)", marginBottom: 16 }}>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: "var(--accent)", letterSpacing: "0.2em", marginBottom: 14 }}>MY TEST RESULTS</div>
          ...
          {testFields.map(({ key, label, unit, type }) => (
            <div key={key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, paddingBottom: 10, borderBottom: "1px solid var(--border)" }}>
              ...
              {type === "bool" ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <input type="number" min="0" value={testVals[key] || ""}
                    ...
                    style={{ ...s.input, width: 64, textAlign: "center", padding: "8px 4px", marginBottom: 0, fontSize: 15, fontWeight: 700 }} />
          ...
          {testError && (
            <div style={{ fontSize: 11, color: "var(--red-dim)", marginBottom: 8, background: "rgba(196,30,30,0.1)", padding: "6px 10px", borderRadius: 4 }}>
              ⚠ {testError}
            </div>
          )}
          <button onClick={saveTests} disabled={savingTests} style={{ ...s.btn, opacity: savingTests ? 0.6 : 1 }}>
      ...
      {nextGaps && (
        <button onClick={() => setShowClaim(true)}
          style={{ ...s.btn, background: nextGaps.level.color, marginBottom: 16, fontFamily: "'Cinzel', serif", letterSpacing: "0.15em" }}>
          {nextGaps.level.icon} CLAIM {nextGaps.level.name}
        </button>
      )}
      {currentRank === 6 && (
        <div style={{ ...s.card, textAlign: "center", padding: 20, borderColor: "#4A0000", background: "rgba(74,0,0,0.1)" }}>
          <div style={{ fontSize: 28, marginBottom: 6 }}>🏛️</div>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: 16, color: "#C0392B" }}>GLADIATOR — ELITE OF THE HOUSE OF STRENGTH</div>
        </div>
      )}
      ...
      <div style={{ textAlign: "center", padding: "20px 0", borderTop: "1px solid var(--border)" }}>
        <div style={{ fontSize: 10, color: "var(--gray2)", letterSpacing: "0.2em", fontFamily: "'Cinzel', serif" }}>FERRUM · SANGUIS · GLORIA</div>
      </div>

      {showClaim && nextGaps && (
        <ClaimLevelModal level={nextGaps.level} authUser={authUser} onClose={() => setShowClaim(false)} onSuccess={() => setShowClaim(false)} />
      )}
    </div>
  );
}
...
export function HallOfStrengthWidget() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const { data: achs } = await supabase.from("level_achievements").select("*")
          .eq("status", "approved").order("approved_at", { ascending: false }).limit(3);
  ...
  return (
    <div style={{ ...s.card, marginBottom: 16, borderColor: "rgba(184,134,11,0.3)", background: "rgba(184,134,11,0.04)" }}>
      <div style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: "var(--accent)", letterSpacing: "0.2em", marginBottom: 10 }}>HALL OF STRENGTH</div>
      {recentAch.map(a => {
        const level = DOM_SILY_LEVELS.find(l => l.id === a.level_id);
        if (!level) return null;
        return (
          <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 8, paddingBottom: 6, marginBottom: 6, borderBottom: "1px solid var(--border)" }}>
            <span style={{ fontSize: 16 }}>{level.icon}</span>
            <div style={{ flex: 1, fontSize: 12 }}>
              <span style={{ color: "var(--accent)", fontWeight: 600 }}>{a.profiles?.name || "Athlete"}</span>
              {" → "}
              <span style={{ fontFamily: "'Cinzel', serif", color: level.color, fontSize: 11 }}>{level.name}</span>
            </div>
          </div>
        );
      })}
...
export function CommunityFeed({ authUser, compact = false }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const { data: achs } = await supabase.from("level_achievements").select("*")
          .eq("status", "approved").order("approved_at", { ascending: false }).limit(compact ? 5 : 30);
        if (achs && achs.length) {
          const ids = [...new Set(achs.map(a => a.user_id))];
          const { data: profs } = await supabase.from("profiles").select("id, name").in("id", ids);
          const nameMap = {};
    ...
    load();
    const sub = supabase.channel("achievements_feed")
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "level_achievements", filter: "status=eq.approved" }, () => load())
      .subscribe();
  ...
  if (loading) return <div style={{ textAlign: "center", padding: 20, color: "var(--gray2)", fontSize: 13 }}>Loading...</div>;
  if (!achievements.length) return (
    <div style={{ textAlign: "center", padding: 24 }}>
      <div style={{ fontSize: 28, marginBottom: 8 }}>🏛️</div>
      <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 16, fontWeight: 900 }}>NO ACHIEVEMENTS YET</div>
      <div style={{ fontSize: 12, color: "var(--gray2)", marginTop: 6 }}>Be the first to claim your rank</div>
    </div>
  );
      ...
      {achievements.map(a => {
        ...
        return (
          <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: "1px solid var(--border)" }}>
            <div style={{ width: 44, height: 44, borderRadius: "50%", flexShrink: 0, background: lv.color + "22", border: `2px solid ${lv.color}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{lv.icon}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>
                <span style={{ color: "var(--accent)" }}>{isOwn ? "You" : (a.profiles?.name || "Athlete")}</span>{" "}unlocked
              </div>
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: 15, fontWeight: 900, color: lv.color, letterSpacing: "0.1em" }}>{lv.name}</div>
              <div style={{ fontSize: 11, color: "var(--gray2)", marginTop: 2 }}>{lv.subtitle}</div>
            </div>
            <div style={{ fontSize: 10, color: "var(--gray2)", flexShrink: 0 }}>{timeAgo(a.approved_at)}</div>
          </div>
        );
      })}
    </div>
...
export function ClaimLevelModal({ level, authUser, onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    try {
      const { error } = await supabase.from("level_achievements").insert({
        user_id: authUser.id, level_id: level.id, level_rank: level.rank,
        status: "pending", video_url: videoUrl || null, note: note || null,
      });
      if (!error) {
        sendPushToUser("a6efb4f6-a5aa-4829-89c3-adb486cf187c",
          `🏆 Level claim: ${level.name}`, `An athlete is claiming ${level.name}`, "achievement", "/");
        setSubmitted(true);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "var(--bg2)", borderRadius: "12px 12px 0 0", padding: 24, width: "100%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        {submitted ? (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>✅</div>
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: 18, color: "var(--accent)" }}>CLAIM SUBMITTED</div>
            <div style={{ fontSize: 13, color: "var(--gray)", marginTop: 8 }}>Coach will review your {level.name} rank</div>
          </div>
        ) : (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              ...
              <div>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 18, fontWeight: 900, color: level.color }}>CLAIM {level.name}</div>
                <div style={{ fontSize: 12, color: "var(--gray)", marginTop: 2 }}>{level.subtitle}</div>
              </div>
            </div>
            <label style={s.label}>VIDEO LINK (optional)</label>
            <input value={videoUrl} onChange={e => setVideoUrl(e.target.value)} placeholder="https://..." style={{ ...s.input, marginBottom: 12 }} />
            <label style={s.label}>NOTE (optional)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Done in person at training..." rows={2} style={{ ...s.input, resize: "none", marginBottom: 16 }} />
            <button onClick={submit} disabled={submitting} style={{ ...s.btn, opacity: submitting ? 0.6 : 1, marginBottom: 10, background: level.color }}>
...
export function RanksCoachView({ athletes }) {
  ...
  const loadPersonAchievements = async (userId) => {
    ...
    try {
      const { data } = await supabase.from("level_achievements").select("*").eq("user_id", userId).eq("status", "approved");
  ...
  const decide = async (id, userId, levelId, levelRank, approve) => {
    ...
    try {
      ...
      if (approve) {
        ...
        sendPushToUser(userId, `🏆 Rank approved: ${lv?.name}!`, `Coach Karlito confirmed your ${lv?.name} rank.`, "achievement", "/");
        sendPushToAllUsers(`🏛️ ${name} reached ${lv?.name}!`, `${name} just earned the ${lv?.name} rank`, "achievement", "/");
      }
      await loadPending();
    } catch (e) {}
    setProcessing(null);
  ...
  const manualAward = async (userId, userName, levelId, levelRank, revoke) => {
    ...
    try {
      if (revoke) {
        const { error } = await supabase.from("level_achievements").delete().eq("user_id", userId).eq("level_id", levelId).eq("status", "approved");
        ...
        if (error) throw error;
        const lv = DOM_SILY_LEVELS.find(l => l.id === levelId);
        sendPushToUser(userId, `🏆 ${lv?.name} awarded by Coach Karlito!`, `Your ${lv?.name} rank has been confirmed.`, "achievement", "/");
      }
      await loadPersonAchievements(userId);
    } catch (e) { setAwardError(e.message || "Error — check level_achievements table in Supabase"); }
    setAwardingLevel(null);
  };

  const allPeople = [
    { id: COACH_ID, name: "👑 Karlito (you)" },
    ...(athletes || []).map(a => ({ id: a.id, name: a.profiles?.name || a.name || a.email?.split("@")[0] || "Athlete" }))
  ];
    ...
    <div>
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {[["pending", `⏳ CLAIMS${pending.length > 0 ? ` (${pending.length})` : ""}`],
          ["manual", "⚔️ AWARD"], ["feed", "🏛️ FEED"]].map(([v, label]) => (
          ...
          {pending.length === 0 ? (
            ...
            return (
              <div key={a.id} style={{ ...s.card, borderColor: lv.color + "44", background: lv.color + "08", marginBottom: 10 }}>
                ...
                {a.video_url && <a href={a.video_url} target="_blank" rel="noreferrer" style={{ display: "block", fontSize: 12, color: "var(--accent)", marginBottom: 8, textDecoration: "underline" }}>🎥 View video</a>}
                {a.note && <div style={{ fontSize: 12, color: "var(--gray)", marginBottom: 10, background: "var(--bg3)", padding: "8px 10px", borderRadius: 4, borderLeft: "2px solid var(--accent)" }}>"{a.note}"</div>}
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => decide(a.id, a.user_id, a.level_id, a.level_rank, true)} disabled={processing === a.id}
                    style={{ ...s.btn, flex: 1, background: lv.color, fontSize: 13, opacity: processing === a.id ? 0.6 : 1 }}>✓ APPROVE</button>
                  <button onClick={() => decide(a.id, a.user_id, a.level_id, a.level_rank, false)} disabled={processing === a.id}
                    style={{ ...s.btnGhost, flex: 1, fontSize: 13, color: "var(--red-dim)", borderColor: "var(--red-dim)" }}>✗ REJECT</button>
                </div>
              </div>
            );
          })}
          ...
          {!selectedPerson ? (
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <button onClick={() => { setSelectedPerson(null); setAwardError(""); }} style={{ ...s.btnGhost, width: "auto", padding: "6px 14px", fontSize: 12 }}>← BACK</button>
                <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 18, fontWeight: 900 }}>{selectedPerson.name}</div>
              </div>
              {awardError && <div style={{ fontSize: 12, color: "var(--red-dim)", marginBottom: 12, background: "rgba(196,30,30,0.1)", padding: "10px 12px", borderRadius: 6 }}>❌ {awardError}</div>}
              {loadingPerson ? (
                ...
                return (
                  <div key={lv.id} style={{ ...s.card, borderColor: hasIt ? lv.color : "var(--border)", background: hasIt ? lv.color + "14" : "var(--bg2)", display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                    <div style={{ fontSize: 26, flexShrink: 0 }}>{lv.icon}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: "'Cinzel', serif", fontSize: 13, fontWeight: 900, color: hasIt ? lv.color : "var(--gray)", letterSpacing: "0.1em" }}>{lv.name}</div>
                      <div style={{ fontSize: 10, color: "var(--gray2)" }}>{lv.subtitle}</div>
                    </div>
                    <button onClick={() => manualAward(selectedPerson.id, selectedPerson.name, lv.id, lv.rank, hasIt)} disabled={isProcessing}
                      style={{ flexShrink: 0, minWidth: 72, background: hasIt ? "transparent" : lv.color, color: hasIt ? "var(--red-dim)" : "#fff", border: hasIt ? "1px solid var(--red-dim)" : "none", borderRadius: 6, padding: "8px 10px", fontFamily: "'Barlow Condensed', sans-serif", fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: isProcessing ? 0.4 : 1 }}>
      ...
      {subview === "feed" && (
        <div>
          ...
          <div style={s.card}><CommunityFeed authUser={{ id: COACH_ID }} compact={false} /></div>
        </div>
      )}
    </div>
  );
