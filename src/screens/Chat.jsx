...
export function ChatScreen({ authUser, isCoach }) {
  ...
  return (
    <div style={s.screen}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, overflowX: "auto" }}>
          {(contacts || []).map(c => (
            <div key={c.id} onClick={() => setSelectedContact(c.id)}
              style={{ ...s.pill(selectedContact === c.id), padding: "8px 14px", whiteSpace: "nowrap" }}>
      ...
      <div style={{ ...s.card, height: "50vh", overflowY: "auto", display: "flex", flexDirection: "column", padding: 12 }}>
        {messages.length === 0 ? (
          <div style={{ textAlign: "center", padding: 40, color: "var(--gray)" }}>No messages yet. Say hi! 👋</div>
        ) : (
          (messages || []).map((msg, i) => {
                  ...
                  <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: 12,
                    ...
                    <div style={{ fontSize: 14, lineHeight: 1.4 }}>
                      {msg.content.startsWith("[MEAL] ") ? (
                        <img src={msg.content.replace("[MEAL] ", "")} alt="Meal"
                          style={{ maxWidth: "100%", borderRadius: 8, marginTop: 4 }}
                          onClick={() => window.open(msg.content.replace("[MEAL] ", ""), "_blank")} />
                      ) : msg.content}
                    </div>
                    <div style={{ fontSize: 10, color: isMe ? "rgba(255,255,255,0.6)" : "var(--gray2)", marginTop: 4, textAlign: "right" }}>
      ...
      {!photoMode && (
        <div style={{ display: "flex", gap: 8, marginTop: 10, alignItems: "center" }}>
          ...
          <input value={newMsg} onChange={e => setNewMsg(e.target.value)}
            ...
            style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderRadius: 6, padding: "12px 14px", color: "var(--white)", fontSize: 15, flex: 1, minWidth: 0, fontFamily: "'DM Sans', sans-serif", outline: "none" }}
          />
